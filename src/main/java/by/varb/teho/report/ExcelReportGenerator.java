...
import by.varb.teho.exception.ExcelReportGeneratorException;
...
public class ExcelReportGenerator<T> {
     ...
     */
    private final static String VAR_NAME = "data";

    public byte[] generateFileFromTemplate(String templateFilePath, List<T> data) throws ExcelReportGeneratorException {
        byte[] reportFile;

        try (InputStream inputStream = new FileInputStream(templateFilePath); ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
