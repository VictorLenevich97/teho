...
**ОБЯЗАТЕЛЬНО**: поменять порт, используемый в `teho-frontend` для общения с бэком на `33657` (файлы `.env` и `src/lib/api.js`). 
В бэкенде менять не нужно, он прописан в скрипте запуска, который будет запускать JAR-файл.
---
## Принцип работы
...
Для создания новой версии, чтобы установщик автоматически обновил существующую установку (изменить и в `win/wix_build.wxs` и в `win/wix_build_with_java/wxs`:
1. Изменить версию внутри флага `Version` тега `Product` (самый верхний уровень) на новую 
2. Изменить UUID внутри флага `UpgradeCode` тега `Product` (самый верхний уровень) на новый, любой случайный UUID
---